...
class CrystalTcGNN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch, material_props=None):
        ...
        if material_props is not None:
            ...
            expected_prop_size = 24  # We have 24 material features
            batch_size = x.size(0)
...
class DeepCrystalTcGNN(torch.nn.Module):
    ...
    def __init__(self, num_node_features: int, num_material_features: int, hidden_dim: int = 64):
        ...
        # Pool reducer to handle concatenated mean and max pooling
        self.pool_reducer = torch.nn.Linear(hidden_dim * 2, hidden_dim)
...
class EnhancedCrystalTcGNN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch, material_props=None):
        ...
        x = self.pool_reducer(x_pooled)
        
        # Handle material properties with better batching
        if material_props is not None:
            expected_prop_size = 24  # Updated to match actual material features count
            batch_size = x.size(0)
...
class SuperEnhancedCrystalTcGNN(torch.nn.Module):
    ...
    def __init__(self, num_node_features: int, num_material_features: int, num_edge_features: int = 6, hidden_dim: int = 256):
        ...
        # Multi-scale edge-aware convolutions (simplified to avoid dimension issues)
        # EdgeConv automatically concatenates node features, so we just define the MLP
        self.edge_conv1 = EdgeConv(torch.nn.Sequential(
            torch.nn.Linear(2 * num_node_features, hidden_dim),
            torch.nn.ReLU(),
...
def main():
    ...
    try:
        ...
        for params in param_combinations:
            ...
            model = EnhancedCrystalTcGNN(  # Use stable enhanced model
                num_node_features, 
